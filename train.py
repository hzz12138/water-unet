...
def train_net(net, device, train_data_path, val_data_path, batch_size, epochs, lr):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if i % 10 == 0:
                print(
                    f"""epoch:{epoch}/{epochs},Loss/train:,{np.mean(total_test_loss)},acc:{np.mean(acc)}""")

            # 给torchvision添加数据
            writer.add_scalar('LOSS/Validation_loss', float(total_test_loss), (epoch + 1))
            writer.add_scalar('ACC/Validation_ACC', float(total_accuracy), (epoch + 1))
...
if __name__ == "__main__":
    ...
    # net.eval()
    train_net(net=net, device=device, train_data_path=train_data_path,val_data_path=val_data_path, batch_size=batch_size, epochs = epochs, lr = lr)
    writer.close()